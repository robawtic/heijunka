...
from .assignment import create_and_add_assignment
from .events import (
...
class Schedule:
    ...
    def generate_assignments(
        ...
        # Prepare work history data with ARO pre-assignments if available
        enhanced_work_history_data = work_history_data.copy() if work_history_data else {}
        
        # Extract ARO pre-assignments from prefetched_data if available
        if prefetched_data and 'aro_pre_assignments' in prefetched_data:
            aro_pre_assignments = prefetched_data['aro_pre_assignments']
            if aro_pre_assignments:
                enhanced_work_history_data['aro_pre_assignments'] = aro_pre_assignments
                logger.info(f"Team {self.team_id}: Found {len(aro_pre_assignments)} ARO pre-assignments in prefetched_data")
        ...
        for period in range(1, self.periods_per_day + 1):
            ...
            period_assignments = cp_model_builder.solve_one_period(
                ...
                aro_data=aro_data,
                team_name=team_name,
                work_history_data=enhanced_work_history_data,
                call_ins=self.call_ins,
                offline_dict=self.offline
            )
