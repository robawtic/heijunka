...
class WorkAssignmentFactory:
    ...
    def create_model_from_entity(
        ...
        is_temporary: bool = False,  # Deprecated
        is_generated: bool = True,   # Deprecated
        end_flag: bool = False,
        logger=None
        ...
        Args:
            ...
            status: Optional status of the work history entry (REGULAR, GENERATED, TEMPORARY, GENERATED_TEMPORARY)
            is_temporary: (Deprecated) Whether this is a temporary assignment
            is_generated: (Deprecated) Whether this assignment was generated by the scheduler
        ...
        try:
            ...
            if model_status is None:
                # For backward compatibility, determine status from boolean flags
                model_is_temporary = metadata.get('is_temporary', is_temporary)
                model_is_generated = metadata.get('is_generated', is_generated)

                if model_is_generated and model_is_temporary:
                    model_status = WorkHistoryStatus.GENERATED_TEMPORARY
                elif model_is_generated:
                    model_status = WorkHistoryStatus.GENERATED
                elif model_is_temporary:
                    model_status = WorkHistoryStatus.TEMPORARY
                else:
                    model_status = WorkHistoryStatus.REGULAR
    ...
    def update_model_from_entity(
        ...
        try:
            ...
            if 'status' in metadata:
                model.status = metadata['status']
            elif 'is_temporary' in metadata or 'is_generated' in metadata:
                # For backward compatibility, determine status from boolean flags
                is_temporary = metadata.get('is_temporary', False)
                is_generated = metadata.get('is_generated', False)

                if is_generated and is_temporary:
                    model.status = WorkHistoryStatus.GENERATED_TEMPORARY
                elif is_generated:
                    model.status = WorkHistoryStatus.GENERATED
                elif is_temporary:
                    model.status = WorkHistoryStatus.TEMPORARY
                else:
                    model.status = WorkHistoryStatus.REGULAR
