...
class TestScheduleFactory(unittest.TestCase):
    ...
    def test_create_schedule_with_assignments(self):
        ...
        mock_employee1.id = 1
        mock_workstation1 = MagicMock()
        mock_workstation1.id = 101
        period1 = SchedulePeriod(date=today, period=1)

        assignment1 = MagicMock(spec=WorkAssignment)
        assignment1.employee = mock_employee1
        assignment1.workstation = mock_workstation1
        assignment1.period = period1

        mock_employee2 = MagicMock()
        mock_employee2.id = 2
        mock_workstation2 = MagicMock()
        mock_workstation2.id = 102
        period2 = SchedulePeriod(date=today, period=2)

        assignment2 = MagicMock(spec=WorkAssignment)
        assignment2.employee = mock_employee2
        assignment2.workstation = mock_workstation2
        assignment2.period = period2
    ...
    def test_create_from_model(self):
        ...
        with patch('domain.factories.employee_factory.EmployeeFactory') as mock_employee_factory, \
            ...
            mock_workstation.id = 101

            # Create a properly structured mock assignment
            period = SchedulePeriod(date=today, period=1)

            mock_assignment = MagicMock(spec=WorkAssignment)
            mock_assignment.employee = mock_employee
            mock_assignment.workstation = mock_workstation
            mock_assignment.period = period
