...
class CacheInvalidationHandler:
    ...
    def handle_aro_assignment_created(self, event: AROAssignmentCreated) -> None:
        ...
        logger.info(f"Cache invalidated for ARO assignment created: {event.employee_id} from team {event.from_team_id} to team {event.to_team_id} on {event.assignment_date}")
    ...
    def handle_aro_assignment_removed(self, event: AROAssignmentRemoved) -> None:
        ...
        self._invalidate_graph_cache_for_date(event.assignment_date, event.period)
        self._invalidate_edge_cost_cache_for_teams(event.from_team_id, event.to_team_id)
        logger.info(f"Cache invalidated for ARO assignment removed: {event.employee_id} from team {event.from_team_id} to team {event.to_team_id} on {event.assignment_date}")
    ...
    def handle_aro_assignment_updated(self, event: AROAssignmentUpdated) -> None:
        ...
        self._invalidate_graph_cache_for_date(event.assignment_date, event.period)
        self._invalidate_edge_cost_cache_for_teams(event.from_team_id, event.to_team_id)
        logger.info(f"Cache invalidated for ARO assignment updated: {event.employee_id} from team {event.from_team_id} to team {event.to_team_id} on {event.assignment_date}")
    ...
    def handle_team_member_added(self, event: TeamMemberAdded) -> None:
        ...
        logger.info(f"Cache invalidated for team member added: {event.employee_id} to team {event.team_id}")
    ...
    def handle_team_member_removed(self, event: TeamMemberRemoved) -> None:
        ...
        """
        self._invalidate_graph_cache_for_team(event.team_id)
        logger.info(f"Cache invalidated for team member removed: {event.employee_id} from team {event.team_id}")
    ...
    def handle_workstation_added_to_team(self, event: WorkstationAddedToTeam) -> None:
        ...
        """
        self._invalidate_graph_cache_for_team(event.team_id)
        logger.info(f"Cache invalidated for workstation added: {event.workstation_id} to team {event.team_id}")
    ...
    def handle_workstation_removed_from_team(self, event: WorkstationRemovedFromTeam) -> None:
        ...
        """
        self._invalidate_graph_cache_for_team(event.team_id)
        logger.info(f"Cache invalidated for workstation removed: {event.workstation_id} from team {event.team_id}")
    ...
    def handle_qualification_added(self, event: QualificationAdded) -> None:
        ...
        logger.info(f"All graph caches invalidated for qualification added: {event.qualification} to employee {event.employee_id}")
    ...
    def handle_qualification_removed(self, event: QualificationRemoved) -> None:
        ...
        # Since we don't know which team the employee belongs to, we need to invalidate all graph caches
        self._invalidate_all_graph_caches()
        logger.info(f"All graph caches invalidated for qualification removed: {event.qualification} from employee {event.employee_id}")
    ...
    def _invalidate_graph_cache_for_date(self, assignment_date: date, period: Optional[int] = None) -> None:
        ...
        if cache_key in self.aro_graph_service._graph_cache:
            ...
            logger.debug(f"Graph cache invalidated for date {assignment_date}, period {period}")
    ...
    def _invalidate_edge_cost_cache_for_teams(self, from_team_id: int, to_team_id: int) -> None:
        ...
        for key in keys_to_remove:
            ...
            logger.debug(f"Edge cost cache invalidated for key {key}")
    ...
    def _invalidate_graph_cache_for_team(self, team_id: int) -> None:
        ...
        logger.debug(f"All graph caches invalidated for team {team_id}")
    ...
    def _invalidate_all_graph_caches(self) -> None:
        ...
        logger.debug("All graph and edge cost caches invalidated")