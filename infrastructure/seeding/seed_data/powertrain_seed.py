...
def seed_powertrain_data(session: Session):
    ...
    # 3) Create or fetch DepartmentModel
    ...
    # 4) Load <department>_groups.json
    ...
    group_objs = {}
    # 5) For each group, create or fetch GroupModel, then process its JSON
    for group_name in groups_json["groups"].keys():
        ...
        team_objs = {}
        # 6) For each team name, create or fetch TeamModel
        for team_name in teams_json["teams"]:
            ...
            # 7) Load workstation.json for this team
            ...
            if ws_json and "workstations" in ws_json:
                for ws_item in ws_json["workstations"]:
                    ...
                    if not ws_obj:
                        ...
                        # Add attributes based on JSON data
                        attributes_to_add = []

                        # Map line_type to mainline/subline attributes
                        line_type = ws_item.get("line_type", "Mainline")
                        if line_type == "Mainline":
                            attributes_to_add.append("mainline")
                        elif line_type in ["Sub-Assembly", "subline"]:
                            attributes_to_add.append("subline")

                        # Map old boolean flags to new attributes
                        if ws_item.get("is_loading_job", False):
                            attributes_to_add.append("loading")
                        if ws_item.get("is_heavy_job", False):
                            attributes_to_add.append("heavy")
                        if ws_item.get("is_key_skill_job", False):
                            attributes_to_add.append("skill_level_3")  # Assuming key skill = advanced skill

                        # Create attribute links
                        for attr_name in attributes_to_add:
                            attr_def = session.query(WorkstationAttributeDefinition).filter_by(name=attr_name).first()
                            if attr_def:
                                # Check if link already exists
                                existing_link = session.query(WorkstationAttributeModel).filter_by(
                                    workstation_id=ws_obj.id,
                                    attribute_id=attr_def.id
                                ).first()
                                if not existing_link:
                                    attr_link = WorkstationAttributeModel(
                                        workstation_id=ws_obj.id,
                                        attribute_id=attr_def.id
                                    )
                                    session.add(attr_link)
                                    print(f"        Added attribute '{attr_name}' to workstation {ws_name}")
            ...
            # 8) Load employee.json for this team
            ...
            if emp_json and "employees" in emp_json:
                for emp_data in emp_json["employees"]:
                    ...
                    # 9) Link KnownStations (EmployeeWorkstationModel)
