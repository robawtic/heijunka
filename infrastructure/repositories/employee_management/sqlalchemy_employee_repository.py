...
class SqlAlchemyEmployeeRepository(BaseSqlAlchemyRepository[Employee, EmployeeModel], EmployeeRepositoryInterface):
    ...
    def get_by_team_ids(self, team_ids: List[int]) -> List[Employee]:
        ...
        self.logger.info(
            f"Retrieving employees for team IDs: {team_ids}",
            extra={
                "event_type": "team_employees_bulk_lookup",
                "team_ids": team_ids,
        ...
        try:
            if not team_ids:
                self.logger.info(
                    "No team IDs provided, returning empty list",
                    extra={
                        "event_type": "team_employees_bulk_lookup_empty",
                        "team_ids": team_ids
                    }
                )
                return []
            ...
            with self.session_scope() as session:
                ...
                employee_count = len(employee_models)
                self.logger.info(
                    f"Found {employee_count} employees for team IDs: {team_ids}",
                    extra={
                        "event_type": "team_employees_bulk_lookup_success",
                        "team_ids": team_ids,
                        "team_count": len(team_ids),
                        "employee_count": employee_count
                    }
            ...
            error_msg = sanitize_exception(e)
            self.logger.error(
                f"Error retrieving employees for team IDs {team_ids}: {error_msg}",
                extra={
                    "event_type": "team_employees_bulk_lookup_error",
                    "team_ids": team_ids,
                    "team_count": len(team_ids),
                    "error_type": type(e).__name__
                }
            )
            raise RepositoryError(f"Error retrieving employees for teams: {error_msg}")
    ...
    def get_by_name(self, name: str) -> Optional[Employee]:
        ...
        try:
            ...
            with self.session_scope() as session:
                employee_model = session.query(EmployeeModel).filter(EmployeeModel.name == name).first()
    ...
    """
        try:
            ...
            with self.session_scope() as session:
                employee_model = session.query(EmployeeModel).filter(EmployeeModel.id == id).first()
                if not employee_model:
                    self.logger.info(
